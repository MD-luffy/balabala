...
$.fn.balabala = function(obj) {
    ...
    //关闭日历选择监听事件
    $.fn.destroyCalendar = function() {
        $('.balabala-select').off(".calendar");
    }
    ...
    function pickerSize() {
        ...
        //日历距顶为25%
        var h = $(window).scrollTop() + $(window).height() * 0.25;
    ...
    function bindEvent(o) {
        function hideCalendar() {
            $(document).off('.bala');
            window.removeEventListener('orientationchange' in window ? 'orientationchange' : 'resize', resHandle, false);
            $('.date-picker').remove();
            $('.date-mask').remove();
        }
        //处理横屏事件
        ...
        // 检测横屏事件，重新设置日历窗口展示
        window.addEventListener('orientationchange' in window ? 'orientationchange' : 'resize', resHandle, false);
        var tStartY = 0;
    ...
    function themeInit(obj) {
        $('.date-picker').css('color', obj.theme || '#000000');
        $('.date-a, .date-s-u li').css('border-bottom', obj.theme ? '1px solid ' + obj.theme : '1px solid #31b6e7');
    ...
    function init(obj) {
        ...
        //日历调用事件
        $('.balabala-select').on('click.calendar', function(evt) {
            // evt.preventDefault();
            var e = evt.target,
                data = $(this).html();
